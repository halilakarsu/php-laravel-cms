                            ...
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-12 col-lg-12">
                                        <form method="POST" action="{{route('page.update',$pageEdit->id)}}" enctype="multipart/form-data">
                                            @csrf
                                            <div class="form-group">
                                                <label for="email2">Yüklü Görsel</label><br>
                                                <img width="30%" src="/backend/images/pages/{{$pageEdit->page_file}}" alt="{{$pageEdit->page_slug}}">
                                              @method('put');
                                            </div>
                                        <div class="form-group">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" name="page_title" id="email2" value="{{$pageEdit->page_title}}">
                                            </div>

                                        <div class="form-group">
                                            <label>Seo Link</label>
                                            <input type="text" class="form-control" name="page_slug" value="{{$pageEdit->page_slug}}">
                                        </div>
                                        <div class="form-group">
                                            <label for="password">Sayfa İçerik</label>
                                            <textarea class="form-control"  name="page_content" >{{ $pageEdit->page_content}}
                                            </textarea>
                                        </div>
                                            <div class="form-group">
                                                <label for="password">Durum</label>
                                                <select class="form-control"  name="page_status">
                                                    <option {{ $pageEdit->page_status==0 ?"selected" : "" }} value="0">Pasif</option>
                                                    <option {{ $pageEdit->page_status==1 ?"selected" : "" }} value="1">Aktif</option>
                                                </select>

                                            </div>
                                      </div>
                                      <input type="hidden" name="oldFile" value="{{$pageEdit->page_file}}">
                                     </div>
